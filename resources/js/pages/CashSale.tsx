...
// Generate years dynamically from current year back to 5 previous years
const generateYears = () => {
...
export default function CashSale({ recentEntries, vatPercentage }: CashSaleProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch('/api/cash-sale/store', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        toast({
          ...
          description: `Cash sale for ${monthName} ${entryData.year} saved successfully.`,
        });
        setEntryData({ ...entryData, totalIncome: "" });
      } else {
        toast({
          title: "Error",
          description: data.message || "Failed to save cash sale.",
          variant: "destructive",
        });
      }
    } catch (error) {
  ...
  const handleView = async () => {
    ...
    try {
      const response = await fetch('/api/cash-sale/show', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
      ...
      if (data.success && data.data) {
        const totalIncome = data.data.income;
        const currentVatPercentage = data.data.vatPercentage || vatPercentage;
        // Calculate VAT and Cash Sale
        // VAT = Total Income * (VAT% / (100 + VAT%))
        const vatAmount = totalIncome * (currentVatPercentage / (100 + currentVatPercentage));
        ...
        toast({
          ...
          description: "Cash sale record found.",
        });
      } else {
        ...
        toast({
          title: "No Data",
          description: `No cash sale record found for ${monthName} ${historyData.year}.`,
          variant: "destructive",
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: "Failed to retrieve cash sale record.",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handlePrint = async () => {
    ...
    try {
      const response = await fetch('/api/cash-sale/generate-pdf', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]')?.getAttribute('content') || '',
      ...
      if (response.ok) {
        ...
        toast({
          title: "PDF Generated",
          description: "Opening cash sale PDF in new window...",
        });
      } else {
        toast({
          title: "Error",
    ...
    <div className="space-y-6">
      ...
      <div className="animate-fade-slide-up">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Cash Sale</h1>
            <p className="text-muted-foreground mt-1">Record and manage monthly cash sales</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {activeTab === "entry" && (
          <div className="card-neumorphic-elevated p-6 space-y-6 animate-card-entrance">
            <h2 className="text-lg font-semibold text-foreground">Record Cash Sale</h2>
        ...
        {activeTab === "history" && (
          <div className="card-neumorphic-elevated p-6 space-y-6 animate-card-entrance">
            <h2 className="text-lg font-semibold text-foreground">Retrieve & Print</h2>
              ...
              <div className="bg-success/5 border border-success/20 rounded-2xl p-6 space-y-4 animate-scale-in">
                ...
                <div className="text-center pb-4 border-b border-success/20">
                  ...
                  <p className="text-2xl font-bold text-foreground">
                    LKR {retrievedData.totalIncome.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-muted-foreground">VAT ({retrievedData.vatPercentage}%)</span>
                  </div>
                  <span className="font-medium text-foreground">
                    LKR {retrievedData.vatAmount.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                ...
                <div className="flex items-center justify-between py-3 px-4 bg-success/10 rounded-xl">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-foreground">Cash Sale</span>
                  </div>
                  <span className="text-xl font-bold text-success">
                    LKR {retrievedData.cashSale.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
            ...
            {recentEntries.length > 0 ? (
              recentEntries.map((entry) => {
                ...
                return (
                  ...
                  >
                    <div>
                      <p className="font-medium text-foreground">{monthName} {entry.year}</p>
                      <p className="text-xs text-muted-foreground">Cash Sale Record</p>
                    </div>
                    <p className="font-semibold text-success">
                      LKR {entry.income.toLocaleString("en-US", { minimumFractionDigits: 2 })}
                    </p>
                  </div>
                );
              })
