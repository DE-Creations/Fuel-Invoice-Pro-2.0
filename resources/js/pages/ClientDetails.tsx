...
export default function ClientDetails({ companies: initialCompanies, fuelCategories }: ClientDetailsProps) {
  ...
  const companyVehicles = vehicles.filter((v) => v.companyId === selectedCompanyId);
  ...
  const handleSaveVehicle = async () => {
    ...
    if (!vehicleForm.vehicleNo.trim() || !vehicleForm.type || !vehicleForm.fuelCategory) {
      toast({
        variant: "destructive",
        title: "Validation Error",
  ...
  const handleDeleteCompany = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setVehicles(vehicles.filter(v => v.companyId !== deleteCompanyId));
        ...
        toast({
          ...
          description: "The company and its vehicles have been successfully deleted.",
        });
      } else {
        toast({
          variant: "destructive",
    ...
    <div className="space-y-6">
      ...
      <div className="animate-fade-slide-up">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-foreground">Client Details</h1>
            <p className="text-muted-foreground mt-1">Manage company and vehicle master data</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="card-neumorphic-elevated p-6 space-y-6 animate-card-entrance">
          ...
          {selectedCompanyId && (() => {
            ...
            return company ? (
              <div className="bg-secondary/30 rounded-xl p-4 space-y-3">
                <div>
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">Nick Name</span>
                  <p className="font-medium text-foreground">{company.nickName || "-"}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-foreground">{company.contact}</p>
                </div>
                <div>
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">VAT No</span>
                  <p className="font-medium text-foreground">{company.vatNo}</p>
                </div>
              </div>
            ) : null;
          })()}
          ...
          <div className="flex gap-3 justify-between">
            ...
            <button
              type="button"
              onClick={() => setDeleteCompanyId(selectedCompanyId)}
        ...
        {/* Vehicle Section */}
        <div className="card-neumorphic-elevated p-6 space-y-6 animate-card-entrance" style={{ animationDelay: "0.1s" }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                if (!selectedCompanyId) {
                  toast({
                    title: "Select Company",
                    description: "Please select a company first to add a vehicle.",
                    variant: "destructive",
                  });
                  return;
                }
          ...
          <SearchableSelect
            ...
            placeholder={selectedCompanyId ? "Select a vehicle" : "Select company first"}
          />
          ...
          {selectedVehicle && (() => {
            ...
            return vehicle ? (
              <div className="bg-accent/10 rounded-xl p-4 space-y-3">
                ...
                <div>
                  <span className="text-xs text-muted-foreground uppercase tracking-wider">Fuel Category</span>
                  <p className="font-medium text-foreground">{vehicle.fuelCategory}</p>
                </div>
              </div>
            ) : null;
          })()}
      ...
      <div className="animate-fade-slide-up" style={{ animationDelay: "0.2s" }}>
        <h2 className="text-lg font-semibold text-foreground mb-4">Vehicle List</h2>
        {!selectedCompanyId ? (
          <div className="card-neumorphic p-12 text-center">
            <FileX className="h-16 w-16 mx-auto text-muted-foreground/40 mb-4" />
            <h3 className="text-lg font-semibold text-foreground mb-2">Select a Company</h3>
            <p className="text-muted-foreground">Select a company to view its vehicles.</p>
          </div>
          ...
          <div className="card-neumorphic p-12 text-center">
            ...
            <p className="text-muted-foreground">This company has no vehicles. Add one to get started.</p>
          </div>
        ) : (
          <DataGrid columns={vehicleColumns} data={companyVehicles} pageSize={15} />
        )}
      </div>
        ...
        <DialogContent className="card-neumorphic-elevated border-none max-w-md">
          ...
          <div className="space-y-4 pt-4">
            ...
            <FloatingInput
              ...
              onChange={(e) => setCompanyForm({ ...companyForm, vatNo: e.target.value })}
            />
        ...
        <DialogContent className="card-neumorphic-elevated border-none max-w-md">
          ...
          <div className="space-y-4 pt-4">
            ...
            <SearchableSelect
              ...
              onChange={(value) => setVehicleForm({ ...vehicleForm, type: value })}
            ...
            <SearchableSelect
              ...
              onChange={(value) => setVehicleForm({ ...vehicleForm, fuelCategory: value })}
            ...
            <div className="flex gap-3 justify-end pt-2">
              ...
              <button
                ...
                disabled={isLoading || !vehicleForm.vehicleNo || !vehicleForm.type || !vehicleForm.fuelCategory}
                className="btn-primary-glow flex items-center gap-2"
              >
                {isLoading ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
      ...
      <AlertDialog open={!!deleteCompanyId} onOpenChange={() => setDeleteCompanyId(null)}>
        <AlertDialogContent className="card-neumorphic-elevated border-none">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this company? This will also delete all associated vehicles. This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
